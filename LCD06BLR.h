/**
 * \mainpage LCD06BLR LCD displayer driver by Holtek HT1621
 *
 * \author Eyes Blue
 * \date 23 November 2019
 * \version 1.0
 * \tableofcontents
 * \section sec_intro LCD06BLR Introduction
 ...
 * if you need implement it. The code modify from Maczinga's code(https://macduino.blogspot.com/2015/02/HT1621.html).
 *
 * \section sec_intro HT1621 Introduction
...
private:
	...
	uint8_t _numBuf[6] = {0, 0, 0, 0, 0, 0};	// The number what the LCD display.
	uint8_t _numSeg[10] = {B00111111, B00000110, B01011011, B01001111, B01100110,	//	The segment of number 0, 1, 2, ...
						   B01101101, B01111101, B00000111, B01111111, B01101111};
	uint8_t _ledSegBuf[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};	// The data map the RAM of HT1621.
	uint8_t _ledBitMap[6][8][2] = {		// Every segment(a - g) of the edage of LCD map to the RAM of HT1621, also map to ledSegBuf.
		//     a		  b			 c			d		   e		  f			 g			dot
		{  {1, 0x04}, {2, 0x40}, {2, 0x04}, {3, 0x80}, {2, 0x08}, {1, 0x08}, {2, 0x80}, {3, 0x40}},
		{  {3, 0x04}, {4, 0x40}, {4, 0x04}, {5, 0x80}, {4, 0x08}, {3, 0x08}, {4, 0x80}, {5, 0x40}},
		{  {5, 0x04}, {6, 0x40}, {6, 0x04}, {7, 0x80}, {6, 0x08}, {5, 0x08}, {6, 0x80}, {7, 0x40}},
		{  {7, 0x04}, {8, 0x40}, {8, 0x04}, {9, 0x80}, {8, 0x08}, {7, 0x08}, {8, 0x80}, {9, 0x40}},
		{  {9, 0x04}, {10, 0x40}, {10, 0x04}, {11, 0x80}, {10, 0x08}, {9, 0x08}, {10, 0x80}, {11, 0x40}},
		{  {11, 0x04}, {12, 0x40}, {12, 0x04}, {13, 0x80}, {12, 0x08}, {11, 0x08}, {12, 0x80}, {0, 0}},
		};
	uint8_t _colonBitMap[2][2] = {{14, 4}, {14, 8}};	// The address of the 2 colon.
...
public:
     ...
     */
    LCD06BLR(uint8_t CSpin, uint8_t RWpin, uint8_t DATApin) : _CS_pin(CSpin), _DATA_pin(DATApin), _RW_pin(RWpin) {};
    /**
     *  \fn void begin(void)
     ...
     * @param The number you want set(0 ~ 9).
     * \warning There is no check to verify if the address is valid.
     */
	void setNum(uint8_t index, uint8_t num);
	/**
     * \fn void setDot(uint8_t index, bool isOn)
     * \brief There are 5 dot on the LCD, set the index of dot on or off.
     * @param Index of the 5 dot of LCD.
     * @param Set it on or off.
     */
	 void setDot(uint8_t index, bool isOn);
	 /**
     * \fn void setColon(uint8_t index, bool isOn)
     * \brief There are 2 colon on the LCD, set the index of colon on or off.
     * @param Index of the 2 colon of LCD.
     * @param Set it on or off.
     */
	 void setColon(uint8_t index, bool isOn);
};
